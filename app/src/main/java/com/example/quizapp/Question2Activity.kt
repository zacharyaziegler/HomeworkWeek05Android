    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        confirmBtn.setOnClickListener {
            ...
            if (selectedOption == -1) { // prevent no option selected
                Toast.makeText(this@Question2Activity, "ERROR! Must select an option", Toast.LENGTH_SHORT).show()
            } else {
                val radioButton = findViewById<RadioButton>(selectedOption)

                if (radioButton.text == "50m") { // if correct
                    earned+=200 // money earned
                    ...
                    Toast.makeText(this@Question2Activity, "CORRECT! You've earned $200!", Toast.LENGTH_SHORT).show()
                } else { // if incorrect
                    Toast.makeText(this@Question2Activity, "WRONG! The correct answer was 50m", Toast.LENGTH_SHORT).show()
                    wrong+=1
                    editor.putInt("wrong", wrong)
                    editor.commit()
                }

                val i = Intent(this, Question3Activity::class.java) // create intent of main activity to launch after animation
                startActivity(i)
                finish()
            }
