    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        confirmBtn.setOnClickListener {
            ...
            if (selectedOption == -1) { // prevent no option selected
                Toast.makeText(this@Question7Activity, "ERROR! Must select an option", Toast.LENGTH_SHORT).show()
            } else {
                val radioButton = findViewById<RadioButton>(selectedOption)

                if (radioButton.text == "3") { // if correct
                    earned+=6400 // money earned
                    ...
                    Toast.makeText(this@Question7Activity, "CORRECT! You've earned $6400!", Toast.LENGTH_SHORT).show()
                } else { // if incorrect
                    Toast.makeText(this@Question7Activity, "WRONG! The correct answer was 3", Toast.LENGTH_SHORT).show()
                    wrong+=1
                    editor.putInt("wrong", wrong)
                    editor.commit()
                }

                val i = Intent(this, StatsActivity::class.java) // create intent of main activity to launch after animation
                startActivity(i)
                finish()
            }
