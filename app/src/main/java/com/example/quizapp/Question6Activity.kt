    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        confirmBtn.setOnClickListener {
            ...
            if (selectedOption == -1) { // prevent no option selected
                Toast.makeText(this@Question6Activity, "ERROR! Must select an option", Toast.LENGTH_SHORT).show()
            } else {
                val radioButton = findViewById<RadioButton>(selectedOption)

                if (radioButton.text == "Sperm Whale") { // if correct
                    earned+=3200 // money earned
                    ...
                    Toast.makeText(this@Question6Activity, "CORRECT! You've earned $3200!", Toast.LENGTH_SHORT).show()
                } else { // if incorrect
                    Toast.makeText(this@Question6Activity, "WRONG! The correct answer was Sperm Whale", Toast.LENGTH_SHORT).show()
                    wrong+=1
                    editor.putInt("wrong", wrong)
                    editor.commit()
                }

                val i = Intent(this, Question7Activity::class.java) // create intent of main activity to launch after animation
                startActivity(i)
                finish()
            }
