    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        confirmBtn.setOnClickListener {
            ...
            if (selectedOption == -1) { // prevent no option selected
                Toast.makeText(this@Question5Activity, "ERROR! Must select an option", Toast.LENGTH_SHORT).show()
            } else {
                val radioButton = findViewById<RadioButton>(selectedOption)

                if (radioButton.text == "John Lennon") { // if correct
                    earned+=1600 // money earned
                    ...
                    Toast.makeText(this@Question5Activity, "CORRECT! You've earned $1600!", Toast.LENGTH_SHORT).show()
                } else { // if incorrect
                    Toast.makeText(this@Question5Activity, "WRONG! The correct answer was John Lennon", Toast.LENGTH_SHORT).show()
                    wrong+=1
                    editor.putInt("wrong", wrong)
                    editor.commit()
                }

                val i = Intent(this, Question6Activity::class.java) // create intent of main activity to launch after animation
                startActivity(i)
                finish()
            }
